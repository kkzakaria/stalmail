...
| A6 Wizard reload (hydratation contexte) | ✅ Pass | **v0.1.25** (#19) — **validé en réel** : reload en phase monitoring → l'aperçu email conserve son domaine (`<user>@getstalmail.com`), hostname/domaine ré-hydratés depuis le serveur. *(Le sélecteur de provider DNS repart à « Manual » au reload : normal, hors périmètre #19.)* |
| B1 Login | ✅ Pass | `kkzakaria@getstalmail.com` → accès boîte. |
| B2 Persistance session | ✅ Pass | rechargement → reste connecté. |
| B3 Expiration→/login | ✅ Pass | session expirée → redirection `/login` (observé). *Pas de bouton « déconnexion » dans l'UI (phase 4e).* |
| C1 Dossiers | ✅ Pass | tous présents (Inbox/Favoris/En attente/Sent/Drafts/Archive/Spam/Trash). *« Archivés » n'est plus fantôme : le mailbox de rôle `archive` est provisionné au 1er archivage — **v0.1.25** (#73).* |
| C2 Compteurs non-lus | ✅ Pass | « marquer non lu » → gras + badge mis à jour. |
| D1 iframe sandbox | ✅ Pass | HTML rendu dans l'iframe `sandbox` (sans `allow-scripts`/`allow-same-origin`) + CSP. Activé par le fix v0.1.22 (préférence HTML, cf. table correctifs). |
| D2 Blocage images | ✅ Pass | Image distante **bloquée** par défaut + bandeau « Images distantes bloquées… » → clic « Afficher les images » affiche l'image (CSP élargie à la demande). **Caveat** : décision non persistée au rechargement → issue **#70**. |
| D3 Pièce jointe listée | ✅ Pass | 3 PJ listées (`.png`/`.xlsx`/`.pdf`, nom + taille) + 📎 dans la liste. **Téléchargement désactivé en dur** (`message-item.tsx:95`, non câblé — fonctionnalité ultérieure, intentionnel). |
| D4 Actions (★/lu/archive/trash/spam) | ✅ Pass | Favori ★ / lu-non-lu / **Corbeille** / **Spam** ✅ ; **Archiver ✅ corrigé v0.1.25** (#73) — **validé en réel** sur instance fraîche : le mail quitte l'inbox et le dossier « Archivés » est créé puis le contient (provisioning à la volée du rôle `archive`). |
| E1 Interne→externe (inbox, pas spam) | ⚠️ Partiel | **Livré et authentifié** vers Gmail, mais classé **spam** au 1er envoi → **réputation d'expéditeur neuve** (domaine + IP sans historique), hors périmètre socle. Bug d'envoi corrigé au passage : capability `submission` manquante pour `Identity/get` → **v0.1.21** (#65). Warm-up requis (marquer non-spam, Postmaster Tools). |
| E2 SPF/DKIM/DMARC pass | ✅ Pass | Confirmé par mail-tester 10/10 (SPF+DKIM+DMARC PASS). |
| E3 Externe→interne (sandbox, image bloquée) | ✅ Pass | Mail Gmail (image distante + lien) reçu **en inbox** (pas spam), rendu en **iframe sandbox**, **image bloquée** + bandeau, lien cliquable (`rel=noopener`, nouvel onglet). Latence 1ʳᵉ livraison = greylisting (normal). |
| E4 Répondre (threading) | ✅ (corrigé) | Pré-remplissage live (À = expéditeur) + logique `buildReplyContext` unit-testée (`Re:`, citation, `inReplyTo`/`references`). **Bug live trouvé** : la réponse rapide restait ouverte après envoi → corrigé **v0.1.24** (#81/#78). Threading Gmail-side à confirmer manuellement. |
| E5 Répondre à tous (auto-exclusion) | ✅ (logique) | `buildReplyContext` unit-testé : `cc = to+cc d'origine MOINS soi-même`. Live non rejoué (nécessite un mail multi-destinataires). |
| E6 Transférer (Fwd: + citation) | ⚠️ Bug | Objet `Fwd:` + champ À vide OK, mais **pas un vrai transfert** : même cité que la réponse, **sans en-tête de transfert (De/Date/Objet/À) ni pièces jointes** → issue **#79** (+ décision : message affiché / fil entier / par-message). |
...
| F2 rDNS | ✅ Pass | PTR `mail.getstalmail.com` stable en **IPv4 et IPv6** (`@1.1.1.1`/`@8.8.8.8`). FCrDNS cohérent (A+AAAA forward). |
| F3 Blacklist | ✅ Pass | Aucune liste noire (inclus dans le 10/10 mail-tester). |

**Synthèse (au 2026-06-25) :** socle validé **du déploiement à l'envoi authentifié** —
install one-command → bootstrap auth → wizard (DNS mail auto + A/AAAA manuels) → certs
Let's Encrypt (Stalwart mail :993 + Caddy webmail :443) → webmail HTTPS valide → **envoi
sortant délivré et 100 % authentifié (mail-tester 10/10, SPF/DKIM/DMARC PASS, rDNS v4+v6)**.
Réception externe→interne OK, lecteur **iframe sandbox + blocage d'images** validé, **fils-réponse**
réparés (#75). Acceptation **A1–A4, B1–B3, C1–C2, D1–D2, E2–E5, E7, F1–F3 ✅** ; **D3 ✅** (PJ listées ;
téléchargement non câblé) ; **D4 ✅** (Archiver corrigé #73, v0.1.25 — voir MAJ ci-dessous) ; **E6 ⚠️** (transfert pas un vrai transfert
#79) ; **E1 ⚠️** (livré mais spam = réputation neuve, hors socle). Tests live du composer (v0.1.24) :
réponse rapide qui restait ouverte → corrigée (#78), Cc/Cci dédoublés (#80). **Restent** : A5 (/setup
re-protégé), E8 (adresse invalide), E9 (rate-limit), vérifs Gmail-side (threading) à rejouer
manuellement, + **#79 (transfert)** à concevoir. Warm-up Gmail en parallèle.

**Mise à jour v0.1.25 (au 2026-06-26) :** lot de fixes validés sur **VPS fraîchement réinitialisé**.
**D4 Archiver ✅** corrigé (#73, provisioning à la volée du rôle `archive`) et **A6 wizard reload ✅**
(#19, ré-hydratation serveur du hostname/domaine) — **tous deux validés en réel**. **#38** (rollback
optimiste concurrent) couvert par test unitaire (race condition, non rejouable manuellement). **#20**
(props run-once) fermée comme obsolète ; **#37** (config `defaultFolders`) fermée car couverte par #73.
...
| **Envoi cassé** : `Identity/get` appelé sans capability `submission` (`unknownMethod`) — jamais testé contre un vrai Stalwart | **v0.1.21** (#65) |
| **Lecteur affichait le `text/plain` au lieu du HTML** (`pickBody`) → corps dégradé/« vide », iframe sandbox + blocage images neutralisés pour les mails multipart | **v0.1.22** (#68) |
| **Fils-réponse illisibles** : ouverture par id email au lieu du `threadId` → `Thread/get` notFound → reader vide + actions HS sur les fils ≥ 2 messages | **v0.1.23** (#75, closes #74) |
| **Réponse rapide reste ouverte** avec son contenu après envoi (pas de reset) | **v0.1.24** (#81, closes #78) |
| **Composer : « Cc/Cci » un seul bouton** ouvrait les deux champs à la fois | **v0.1.24** (#82, closes #80) |
| **« Transférer » n'est pas un vrai transfert** (même cité que la réponse, sans en-tête ni pièces jointes) | issue **#79** (brainstorm/spec) |
| **Action « Archiver » cassée** : Stalwart ne provisionne pas de mailbox de rôle `archive` | **v0.1.25** (#73, provisioning à la volée + re-résolution concurrence ; RFC 8621 §2 — closes #37) |
| **Contexte wizard perdu au reload** en phase monitoring (email tronqué `marie@`, domaine vide) | **v0.1.25** (#19, ré-dérivation serveur `setupContextFn`) |
| **Rollback optimiste concurrent** : un échec annulait la modif confirmée d'une action concurrente | **v0.1.25** (#38, re-sync `invalidateQueries`) |
| **Props périmées run-once** dans les étapes wizard (DnsStep/AccountStep) | fermée **obsolète** (#20 — refactor : appels depuis `onSubmit`) |
...
| « Afficher les images » non persisté (rebloqué au rechargement) | issue **#70** |
