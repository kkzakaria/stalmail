...
| E4 Répondre (threading) | ✅ (logique) | Pré-remplissage **live** confirmé (À = expéditeur). Logique `buildReplyContext` **unit-testée** : objet `Re:` (pas de double préfixe), citation sanitisée, `inReplyTo`/`references`. **Réserve** : envoi live depuis QuickReply non automatisé (éditeur RTE) + threading Gmail-side à confirmer manuellement. |
| E5 Répondre à tous (auto-exclusion) | ✅ (logique) | `buildReplyContext` unit-testé : `cc = to+cc d'origine MOINS soi-même`. Live non rejoué (nécessite un mail multi-destinataires + envoi RTE). |
| E6 Transférer (Fwd: + citation) | ✅ (logique) | `buildReplyContext` unit-testé : objet `Fwd:`, to/cc vides, citation. Live non rejoué (envoi RTE). |
...
réparés (#75). Acceptation **A1–A4, B1–B3, C1–C2, D1–D2, E2–E3, E7, F1–F3 ✅** ; **D3 ✅** (PJ listées ;
téléchargement non câblé) ; **E4–E6 ✅ (logique unit-testée + pré-remplissage live)** ; **D4 ⚠️** (Archiver
cassé #73) ; **E1 ⚠️** (livré mais spam = réputation neuve, hors socle). **Restent** : A5 (/setup re-protégé),
E8 (adresse invalide), E9 (rate-limit), + envoi **live** depuis QuickReply et vérifs Gmail-side
(threading/Fwd:) à rejouer manuellement. Warm-up Gmail en parallèle (non-spam, Postmaster Tools).
