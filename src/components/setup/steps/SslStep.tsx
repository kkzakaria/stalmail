...
type Phase = 'configuring' | 'monitor' | 'error'

interface Props {
  hostname: string
  contactEmail: string
  configureAcme: (i: { hostname: string; contactEmail: string }) => Promise<{ ok: true }>
...
export function SslStep({
  ...
  const badgeVariant =
    status === 'failed' ? 'destructive' : status === 'valid' ? 'success' : 'pending'

  return (
    <div className="step-body">
      <StepHeader title={t('wizard.ssl.title')} sub={t('wizard.ssl.subtitle')} />
        ...
        <>
          <div className="recap">
            <div className="recap-row">
              ...
              <span className="recap-value">{t('wizard.ssl.providerValue')}</span>
            </div>
            <div className="recap-row">
