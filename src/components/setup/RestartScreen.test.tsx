...
describe('RestartScreen', () => {
  ...
  it('shows the timeout subtitle and a Retry button once the soft timeout elapses', async () => {
    const poll = vi.fn(async () => ({ step: 'collect' }))
    const onReady = vi.fn()
    wrap(<RestartScreen poll={poll} intervalMs={5} timeoutMs={0} onReady={onReady} />)
    await waitFor(() =>
      expect(
        screen.getByText('Cela prend plus de temps que prévu. Vous pouvez réessayer.'),
      ).toBeInTheDocument(),
    )
