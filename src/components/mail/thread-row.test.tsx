...
const base: AppThread = {
  ...
  messageCount: 1,
  receivedAt: "2026-06-12T08:00:00",
  unread: false,
  starred: false,
  hasAttachment: false,
  mailboxIds: ["mi"],
...
describe("ThreadRow", () => {
  it("point non-lu visible si unread", () => {
    const { container } = render(
      <ThreadRow thread={{ ...base, unread: true }} folder="inbox" now={now} />
    )
  ...
  it("icône étoile pleine si starred", () => {
    const { container } = render(
      <ThreadRow thread={{ ...base, starred: true }} folder="inbox" now={now} />
    )
  ...
  it("trombone si hasAttachment", () => {
    const { container } = render(
      <ThreadRow
        thread={{ ...base, hasAttachment: true }}
        folder="inbox"
        now={now}
      />
    )
  ...
  it("rend un skeleton quand thread est undefined", () => {
    const { container } = render(
      <ThreadRow thread={undefined} folder="inbox" now={now} />
    )
  ...
  it("appelle onOpen au clic avec le threadId (pas l'id email)", () => {
    // Régression : ouvrir un fil doit passer le threadId (t1) — pas l'id email (e1).
    // Sinon Thread/get(emailId) renvoie notFound pour les fils-réponse (réponse en
    // inbox, original en sent → emailId ≠ threadId) → reader vide + actions HS.
    const onOpen = vi.fn()
    const { container } = render(
      <ThreadRow thread={base} folder="inbox" now={now} onOpen={onOpen} />
    )
  ...
  it("applique la classe sel si selected", () => {
    const { container } = render(
      <ThreadRow thread={base} folder="inbox" now={now} selected />
    )
