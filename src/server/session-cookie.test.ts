...
const headers = (map: Record<string, string | undefined>) =>
  vi
    .mocked(getRequestHeader)
    .mockImplementation((n: string) => map[n.toLowerCase()])
...
describe("session cookie", () => {
  it("uses the __Host- name in production and reads/writes/clears it", () => {
    ...
    clearSid()
    expect(deleteCookie).toHaveBeenCalledWith("__Host-stalmail_session", {
      path: "/",
      secure: true,
    })
  })
...
describe("assertSameOrigin", () => {
  ...
  it("throws when the scheme does not match (http Origin against https host)", () => {
    headers({
      origin: "http://mail.x/login",
      "x-forwarded-host": "mail.x",
...
describe("assertSameOriginStrict", () => {
  it("throws when both Origin and Referer are absent (unlike assertSameOrigin)", () => {
    headers({})
    expect(() => assertSameOriginStrict()).toThrow(
      "cross-origin request rejected: missing Origin/Referer"
    )
  })
  it("passes for a same-origin request with Origin header", () => {
    headers({ origin: "https://mail.x/login", "x-forwarded-host": "mail.x" })
    expect(() => assertSameOriginStrict()).not.toThrow()
  })
  it("passes for a same-origin request with Referer header only", () => {
    headers({ referer: "https://mail.x/login", "x-forwarded-host": "mail.x" })
    expect(() => assertSameOriginStrict()).not.toThrow()
  })
  it("throws on a cross-origin Origin", () => {
    headers({ origin: "https://evil.x", host: "mail.x" })
    expect(() => assertSameOriginStrict()).toThrow()
  })
  it("throws on a cross-origin Referer when Origin is absent", () => {
