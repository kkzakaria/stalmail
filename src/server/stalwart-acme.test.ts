...
describe('configureAcme', () => {
  it('creates a TLS-ALPN-01 provider (contact map) then flips the domain (SAN map)', async () => {
    mj.mockResolvedValueOnce([['x:AcmeProvider/set', { created: { p1: { id: 'prov1' } } }, '0']])
    ...
    expect(create.challengeType).toBe('TlsAlpn01')
    ...
    const certMgmt = (
      args2.update as { dom1: { certificateManagement: Record<string, unknown> } }
  ...
  it('throws when the provider create is rejected (notCreated, no created)', async () => {
    mj.mockResolvedValueOnce([
      ['x:AcmeProvider/set', { notCreated: { p1: { type: 'invalidProperties' } } }, '0'],
    ])
    await expect(
      configureAcme({ domainId: 'dom1', hostname: 'mail.dupont.fr', contactEmail: 'admin@dupont.fr' }),
  ...
  it('throws when the domain update is rejected (notUpdated, no updated)', async () => {
    mj.mockResolvedValueOnce([['x:AcmeProvider/set', { created: { p1: { id: 'prov1' } } }, '0']])
    mj.mockResolvedValueOnce([
      ['x:Domain/set', { notUpdated: { dom1: { type: 'invalidProperties' } } }, '0'],
    ])
    await expect(
      configureAcme({ domainId: 'dom1', hostname: 'mail.dupont.fr', contactEmail: 'admin@dupont.fr' }),
    ).rejects.toBeInstanceOf(JmapError)
...
describe('getAcmeStatus', () => {
  it("returns 'pending' when the AcmeRenewal task is Pending", async () => {
    mj.mockResolvedValueOnce([
      ...
      ['x:Task/get', { list: [{ '@type': 'AcmeRenewal', status: { '@type': 'Pending' } }] }, '1'],
    ])
  ...
  it("returns 'failed' when the AcmeRenewal task is Failed", async () => {
    mj.mockResolvedValueOnce([
      ['x:Task/query', { ids: ['t1'] }, '0'],
      ['x:Task/get', { list: [{ '@type': 'AcmeRenewal', status: { '@type': 'Failed' } }] }, '1'],
    ])
  ...
  it("returns 'valid' when there is no AcmeRenewal task", async () => {
    mj.mockResolvedValueOnce([
      ['x:Task/query', { ids: [] }, '0'],
      ['x:Task/get', { list: [{ '@type': 'SomethingElse', status: { '@type': 'Pending' } }] }, '1'],
    ])
