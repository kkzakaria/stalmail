...
describe("isBootstrapMode", () => {
  it("returns true when probing a domain yields the bootstrap forbidden error", async () => {
    mj.mockResolvedValue([
      ["error", { type: "forbidden", description: "bootstrap mode" }, "0"],
    ])
...
describe("getBootstrap", () => {
  it("returns the singleton object", async () => {
    mj.mockResolvedValue([
      [
        "x:Bootstrap/get",
        { list: [{ id: "singleton", defaultDomain: "example.org" }] },
        "0",
      ],
    ])
    expect(await getBootstrap()).toEqual({
      id: "singleton",
  ...
  it("updates the singleton and returns the generated admin credentials", async () => {
    mj.mockResolvedValue([
      [
        ...
        {
          updated: {
            singleton: { username: "admin@exemple.fr", secret: "gen" },
          },
        },
        "0",
      ],
...
describe("getServerHostname", () => {
  it("renvoie le serverHostname du singleton bootstrap", async () => {
    mj.mockResolvedValueOnce([
      [
        "x:Bootstrap/get",
        { list: [{ serverHostname: "mail.exemple.fr" }] },
        "0",
      ],
    ])
  ...
  it("renvoie une chaîne vide quand le champ est absent", async () => {
    mj.mockResolvedValueOnce([["x:Bootstrap/get", { list: [{}] }, "0"]])
